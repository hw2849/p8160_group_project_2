...
newtonraphson <- function(dat, func, start, tol=1e-10, maxiter = 20000) {
  ...
  while(i < maxiter && abs(stuff$loglik - prevloglik) > tol) {
    ...
    prevstuff <- stuff
    stuff <- func(dat, curbeta)        # log-lik, gradient, Hessian
    gamma <- 0.01
    
    while(max(eigen(stuff$hess)$value)>0){
      stuff$Hess = stuff$hess - diag(20) * gamma
      gamma <- gamma + 0.01
    }
    
    if (stuff$loglik > prevloglik)
    {
        res <- rbind(res, c(i, stuff$loglik, curbeta))# Add current values to results matrix
    } else 
    {
      lambda <- 1
      while (stuff$loglik < prevloglik) {
        lambda <- lambda / 2 # step-halving
        curbeta <- prev - lambda * solve(prevstuff$hess) %*% prevstuff$grad
        stuff <- func(dat, curbeta)        # log-lik, gradient, Hessian
      }
        res <- rbind(res, c(i, stuff$loglik, curbeta))# Add current values to results matrix
    }
...
=======
>>>>>>> 78ea447f5ee1bc4aea1da5196d417feacfb8a26c
