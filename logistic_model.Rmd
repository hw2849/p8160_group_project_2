...
=======
  mutate(diagnosis = recode(diagnosis, "M" = 1, "B" = 0))
>>>>>>> b4b7a99ab1a5903eef4004a888237bb54387d054
...
lambdamax <- max(abs(t(x.matrix) %*% y.matrix))
lambdamax <- lambdamax/nrow(y.matrix)
...
```{r}
beta <- pathwise_sol[49, 2:32]
theta <- x_train_stan %*% t(beta)
p <- exp(theta) / (1 + exp(theta))
y.pred <- ifelse(p > 0.5, 1, 0)
mean(y.pred == y_train)
auc(y_train, p)

y_train <- breast_train %>% mutate(diagnosis = factor(diagnosis))
