...
```{r}
log_model = glm(diagnosis ~ ., family = binomial("logit"), data  = breast_train, start = rep(0, 31))
summary(log_model)
```
...
predict <- function(x, y, betavec, beta0) {
  theta <- x %*% betavec
  p <- exp(theta) / (1 + exp(theta))
  y.pred <- ifelse(p > 0.5, 1, 0)
...
beta = as.vector(log_model$coefficients)
