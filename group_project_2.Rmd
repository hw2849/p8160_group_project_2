...
maxlambda <- max(lambda_df$V1) / nrow(y)
maxlambda
```
...
cv = function(data, lambda) {
  ...
  mse_lambda <- rowMeans(mse)
  return(cbind(lambda, mse_lambda))
}

cv_test = cv(data = breast_dat, lambda = seq(5, 0, length = 20))
```
