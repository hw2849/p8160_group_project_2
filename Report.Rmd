...
\item Step 3: Define a quadratic approximated objective function \[L(\beta_0, \textbf{$\beta$}, \lambda)\] for $\lambda_k$ using the estimated parameter at $\lambda_{k-1}$ ($\lambda_{k-1} > \lambda_k$).
\item Step 4: Run coordinate descendent algorithm to find the optimization defined in Step 3.
\end{itemize}
